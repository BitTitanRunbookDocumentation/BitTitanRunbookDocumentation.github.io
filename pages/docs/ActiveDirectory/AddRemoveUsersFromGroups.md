...
### SYNOPSIS
```
    (BETA) Adds or removes users from groups in Active Directory
```
### DESCRIPTION
```
...
```
    Usage option 1: Add or remove a user from one or more groups
        UserIdentity
            ...
            This identifies the user which will be added/removed from the groups.
    ...
    Usage option 2: Add or remove users from groups
        UsersCsv
...
```
    ActiveDirectoryAddRemoveUsersFromGroupsCsv
        A CSV string containing information about the users which were added/removed from the groups,
        as well as an additional column 'MembershipUpdated' to indicate if the users were successfully
        added/removed from the groups.
